...
export const setNextDealer = (state: RoundSlice & Partial<PlayersSlice>) => {
  ...
  const nextDealerIndex = currentDealerIndex === -1 ? 0 : (currentDealerIndex + 1) % players.length;
...
export const getOpponentPlayersScore = (
  ...
  return playersScores?.filter((playerScore) => playerScore.playerId !== roundPlayer?.id);
};

export const getOpponentTeamScore = (roundPlayer: Player | null, teamsScores?: TeamScore[]) => {
  return teamsScores?.filter((teamScore) => teamScore.teamId !== roundPlayer?.teamId);
};
