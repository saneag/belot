...
const handlePlayersScoreChange = ({
  ...
  const opponentsCalculatedRoundScore = {
    ...
    playersScores: playersScores.map((playerScore) => ({
      ...playerScore,
      score: playerScore.playerId === opponent.playerId ? newScoreValue : playerScore.score,
    })),
  };
  ...
  return {
    ...
    playersScores: opponentsCalculatedRoundScore.playersScores.map((playerScore) => ({
      ...playerScore,
      score: playerScore.playerId === roundPlayer?.id ? roundPlayerScore : playerScore.score,
    })),
  };
};
...
const handleTeamsScoreChange = ({
  ...
  return {
    ...
    teamsScores: teamsScores.map((teamScore) => ({
      ...teamScore,
      score: teamScore.teamId === opponent.teamId ? newScoreValue : scoreDifference,
    })),
  };
};
